...
// ========== 도움말 ==========
const HelpGuide = ({ onNavigate }) => {
  const [openFaq, setOpenFaq] = useState(null);

  // 해당 섹션으로 이동하는 버튼
  const GoBtn = ({ section, label }) => (
    <button onClick={() => onNavigate(section)}
      className="inline-flex items-center gap-1.5 px-4 py-2 bg-[#059669] text-white rounded-full text-xs font-semibold hover:bg-[#047857] transition mt-3">
      {label} 바로가기 →
    </button>
  );

  const faqs = [
    { q: '데이터가 날아갔어요 😱', a: '전에 "JSON 내보내기"한 파일이 있으면 "JSON 가져오기"로 복원하세요. 파일이 없으면 다시 입력해야 해요. 매번 저장이 중요합니다!' },
    { q: '법인이랑 개인 파일을 따로 만들어야 해요?', a: '아뇨! 하나의 파일에 다 들어있어요. 위에서 탭만 전환하면 됩니다.' },
    { q: '"희망 납부세액"을 얼마로 넣어요?', a: '편하게 감당할 수 있는 금액을 넣으세요. 세무사님과 1년 목표를 정하고 12로 나누는 것도 좋아요.' },
    { q: '고정비가 바뀌면 다시 넣어야 해요?', a: '기본값을 바꾸면 모든 달에 적용돼요. 특정 달만 다르게 하려면 "월 고정비 조정"으로 그 달만 수정하세요.' },
    { q: '스마트폰으로도 돼요?', a: '네! 폰 브라우저로 접속하면 돼요. 홈 화면에 추가하면 앱처럼 쓸 수 있어요.' },
  ];

  return (
    <div className="space-y-6">
      {/* 히어로 */}
      <div className="bg-gradient-to-br from-[#059669] to-[#047857] rounded-3xl p-6 sm:p-8 text-white">
        <p className="text-4xl mb-3">🐷</p>
        <h2 className="text-2xl sm:text-3xl font-bold mb-2" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
          피그건 세무장부<br/>사용 가이드
        </h2>
        <p className="text-sm opacity-80 mt-3 leading-relaxed">
          회계를 몰라도 괜찮아요!<br/>
          "세금 얼마 내고 싶어?" 한마디면 끝입니다.
        </p>
      </div>

      {/* 핵심 개념 — 비주얼 */}
      <Card>
        <p className="text-xs font-bold text-[#059669] mb-2">핵심 개념</p>
        <h3 className="text-lg font-bold mb-4" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>이 장부는 거꾸로 계산해요</h3>

        {/* 비주얼 비교 */}
        <div className="bg-[#F9FAFB] rounded-2xl p-4 mb-3">
          <p className="text-[10px] font-bold text-[#9CA3AF] mb-2">일반 장부</p>
          <div className="flex items-center gap-2 text-sm flex-wrap">
            <span className="bg-white rounded-xl px-3 py-2 font-medium">💰 번 돈</span>
            <span className="text-[#9CA3AF]">→</span>
            <span className="bg-white rounded-xl px-3 py-2 font-medium">💸 쓴 돈</span>
            <span className="text-[#9CA3AF]">→</span>
            <span className="bg-white rounded-xl px-3 py-2 font-medium">🧮 세금 계산</span>
          </div>
        </div>

        <div className="bg-[#ECFDF5] rounded-2xl p-4 border-2 border-[#059669]">
          <p className="text-[10px] font-bold text-[#059669] mb-2">피그건 장부 ⭐</p>
          <div className="flex items-center gap-2 text-sm flex-wrap">
            <span className="bg-white rounded-xl px-3 py-2 font-bold text-[#059669]">🎯 세금 목표</span>
            <span className="text-[#059669]">→</span>
            <span className="bg-white rounded-xl px-3 py-2 font-bold text-[#059669]">📊 필요 비용 계산</span>
            <span className="text-[#059669]">→</span>
            <span className="bg-white rounded-xl px-3 py-2 font-bold text-[#059669]">💡 부족분 알림</span>
          </div>
        </div>
      </Card>
      ...
      {/* 메뉴 안내 — 카드 그리드 */}
      <div>
        <p className="text-xs font-bold text-[#059669] mb-2 px-1">메뉴 안내</p>
        <h3 className="text-lg font-bold mb-4 px-1" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>각 메뉴가 하는 일</h3>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {[
            { icon: '📊', name: '대시보드', desc: '한눈에 요약 보기', sec: '대시보드', color: 'from-emerald-50 to-emerald-100' },
            { icon: '📝', name: '월별입력', desc: '매출·비용 넣기', sec: '월별입력', color: 'from-blue-50 to-blue-100' },
            { icon: '🔫', name: '중고매입', desc: '당근/중고나라 기록', sec: '중고매입', color: 'from-orange-50 to-orange-100' },
            { icon: '⚙️', name: '고정비', desc: '월세·급여 한번 설정', sec: '고정비', color: 'from-purple-50 to-purple-100' },
            { icon: '🏗️', name: '감가상각', desc: '장비 나눠서 비용처리', sec: '감가상각', color: 'from-amber-50 to-amber-100' },
            { icon: '💰', name: '부가세', desc: '분기별 세금 미리보기', sec: '부가세', color: 'from-red-50 to-red-100' },
            { icon: '📤', name: '데이터', desc: '저장 & 불러오기', sec: '데이터', color: 'from-cyan-50 to-cyan-100' },
          ].map((m, i) => (
            <button key={i} onClick={() => onNavigate(m.sec)}
              className={`bg-gradient-to-br ${m.color} rounded-2xl p-4 text-left hover:scale-[1.02] transition-transform`}>
              <p className="text-2xl mb-2">{m.icon}</p>
              <p className="font-bold text-sm text-[#111827]">{m.name}</p>
      ...
      {/* 대시보드 — 역산 시뮬레이션 비주얼 */}
      <Card>
        <p className="text-xs font-bold text-[#059669] mb-2">📊 대시보드</p>
        <h3 className="text-lg font-bold mb-4" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>⭐ 역산 시뮬레이션 사용법</h3>
        ...
        <div className="bg-[#F9FAFB] rounded-2xl p-4 mb-4">
          <div className="bg-white rounded-2xl p-4 space-y-3">
            {/* 스텝 1 */}
            <div className="flex items-center gap-3">
              <span className="w-10 h-10 rounded-full bg-[#059669] text-white flex items-center justify-center text-lg font-bold shrink-0">1</span>
              <div className="flex-1">
                <p className="text-xs text-[#9CA3AF] mb-1">이달 내고 싶은 세금</p>
                <div className="bg-[#F9FAFB] rounded-xl px-4 py-2.5 flex items-center">
                  <span className="text-lg font-bold text-[#111827]" style={{ fontFamily: "'Plus Jakarta Sans'" }}>300,000</span>
                  <span className="text-sm text-[#9CA3AF] ml-1">원</span>
                </div>
              </div>
            </div>
            ...
            {/* 스텝 2 — 결과 */}
            <div className="flex items-center gap-3">
              <span className="w-10 h-10 rounded-full bg-[#059669] text-white flex items-center justify-center text-lg font-bold shrink-0">2</span>
              <div className="flex-1 grid grid-cols-2 gap-2">
                <div className="bg-[#F9FAFB] rounded-xl p-3">
                  <p className="text-[10px] text-[#9CA3AF]">필요 비용</p>
                  <p className="font-bold text-sm" style={{ fontFamily: "'Plus Jakarta Sans'" }}>15,500,000원</p>
                </div>
                <div className="bg-[#F9FAFB] rounded-xl p-3">
                  <p className="text-[10px] text-[#9CA3AF]">현재 비용</p>
                  <p className="font-bold text-sm text-[#059669]" style={{ fontFamily: "'Plus Jakarta Sans'" }}>11,166,000원</p>
                </div>
              </div>
            </div>
            ...
            {/* 스텝 3 — 부족분 */}
            <div className="flex items-center gap-3">
              <span className="w-10 h-10 rounded-full bg-[#EF4444] text-white flex items-center justify-center text-lg font-bold shrink-0">3</span>
              <div className="flex-1 bg-[#FEF2F2] rounded-xl p-3 border border-[#EF4444]">
                <p className="text-[10px] text-[#9CA3AF]">비용 부족분</p>
                <p className="font-bold text-lg text-[#EF4444]" style={{ fontFamily: "'Plus Jakarta Sans'" }}>4,334,000원</p>
                <p className="text-[10px] text-[#6B7280] mt-1">이만큼 비용을 더 만들어야 해요!</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#FFFBEB] rounded-2xl p-4">
          <p className="font-bold text-sm mb-3">💡 부족하면 이렇게!</p>
          <div className="grid grid-cols-3 gap-2">
            <div className="bg-white rounded-xl p-3 text-center">
              <p className="text-2xl mb-1">🍽️</p>
              <p className="text-[10px] font-bold">법인카드로 밥</p>
            </div>
            <div className="bg-white rounded-xl p-3 text-center">
              <p className="text-2xl mb-1">⛽</p>
              <p className="text-[10px] font-bold">법인카드로 주유</p>
            </div>
            <div className="bg-white rounded-xl p-3 text-center">
              <p className="text-2xl mb-1">🎁</p>
              <p className="text-[10px] font-bold">대표 상여금</p>
            </div>
          </div>
        </div>
        <GoBtn section="대시보드" label="대시보드" />
      </Card>

      {/* 월별입력 — 비주얼 */}
      <Card>
        <p className="text-xs font-bold text-[#059669] mb-2">📝 월별입력</p>
        <h3 className="text-lg font-bold mb-4" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>카드 전표 보고 숫자만 넣으면 끝!</h3>

        {/* 매출 입력 미니 시뮬 */}
        <div className="bg-[#F9FAFB] rounded-2xl p-4 mb-4">
          <p className="text-xs font-bold text-[#9CA3AF] mb-3">이런 식으로 넣어요</p>
          <div className="space-y-2">
            <div className="bg-white rounded-xl p-3 flex items-center justify-between">
              <span className="text-sm flex items-center gap-2">💳 카드매출</span>
              <span className="font-bold text-sm" style={{ fontFamily: "'Plus Jakarta Sans'" }}>8,000,000</span>
            </div>
            <div className="bg-white rounded-xl p-3 flex items-center justify-between">
              <span className="text-sm flex items-center gap-2">💵 현금매출</span>
              <span className="font-bold text-sm" style={{ fontFamily: "'Plus Jakarta Sans'" }}>2,000,000</span>
            </div>
            <div className="bg-[#ECFDF5] rounded-xl p-3 flex items-center justify-between">
              <span className="text-sm font-bold text-[#059669]">합계</span>
              <span className="font-bold text-[#059669]" style={{ fontFamily: "'Plus Jakarta Sans'" }}>10,000,000원</span>
            </div>
          </div>
        </div>

        {/* 부가세 공제 비주얼 */}
        <p className="text-xs font-bold text-[#9CA3AF] mb-2">비용 항목별 부가세 공제 여부</p>
        <div className="grid grid-cols-2 gap-2 mb-3">
          <div className="bg-[#ECFDF5] rounded-xl p-3">
            <p className="text-sm font-bold text-[#059669] mb-2">✅ 세금 깎아줌</p>
          ...
          <div className="bg-[#FEF2F2] rounded-xl p-3">
            <p className="text-sm font-bold text-[#EF4444] mb-2">❌ 안 깎아줌</p>
            <div className="space-y-1 text-xs text-[#6B7280]">
              <p>👤 급여/4대보험</p>
              <p>🔫 개인한테 산 중고</p>
              <p>💵 현금매입 (영수증X)</p>
        ...
        <GoBtn section="월별입력" label="월별입력" />
      </Card>

      {/* 중고매입 — 비주얼 */}
      <Card>
        <div className="flex items-center gap-2 mb-2">
          <p className="text-xs font-bold text-[#059669]">🔫 중고매입</p>
          <span className="text-[10px] bg-[#ECFDF5] text-[#059669] px-2 py-0.5 rounded-full font-bold">법인만</span>
        </div>
        <h3 className="text-lg font-bold mb-4" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>당근/중고나라 구매 기록</h3>
          ...
          <div className="bg-white rounded-xl p-4 space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="font-bold text-sm">M4A1 에어소프트건</p>
                  <p className="text-[10px] text-[#9CA3AF]">2026.01.05 · 당근마켓</p>
                </div>
              </div>
              <p className="font-bold text-sm" style={{ fontFamily: "'Plus Jakarta Sans'" }}>350,000원</p>
            </div>
            <div className="flex gap-2">
              <span className="text-[10px] bg-[#F9FAFB] px-2 py-1 rounded-lg">💳 계좌이체</span>
              <span className="text-[10px] bg-[#FEF2F2] text-[#EF4444] px-2 py-1 rounded-lg font-bold">❌ 증빙없음</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#FEF2F2] rounded-2xl p-4 mb-3">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="font-bold text-sm text-[#EF4444]">증빙 없으면 세금 더 내요!</p>
              <p className="text-xs text-[#6B7280] mt-1">현금으로 살 때는 <strong>간이영수증 + 상대방 신분증</strong> 꼭 받아두세요</p>
            </div>
          </div>
        </div>
        <GoBtn section="중고매입" label="중고매입" />
      </Card>

      {/* 고정비 + 감가상각 — 합침 */}
      <Card>
        <p className="text-xs font-bold text-[#059669] mb-2">⚙️ 고정비 + 🏗️ 감가상각</p>
        <h3 className="text-lg font-bold mb-4" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>한번 설정하면 자동!</h3>

        <div className="grid grid-cols-2 gap-3 mb-4">
          <div className="bg-[#F9FAFB] rounded-2xl p-4">
            <p className="font-bold text-sm mb-3">⚙️ 고정비</p>
            <div className="space-y-2">
              <div className="bg-white rounded-lg px-3 py-2 flex items-center justify-between">
                <span className="text-xs">🔒 월세</span>
                <span className="text-xs font-bold">80만</span>
              </div>
              <div className="bg-white rounded-lg px-3 py-2 flex items-center justify-between">
                <span className="text-xs">🔒 급여</span>
                <span className="text-xs font-bold">300만</span>
              </div>
              <div className="bg-white rounded-lg px-3 py-2 flex items-center justify-between">
                <span className="text-xs">🔒 4대보험</span>
                <span className="text-xs font-bold">45만</span>
              </div>
            </div>
            <p className="text-[10px] text-[#9CA3AF] mt-2 text-center">1번 넣으면 12개월 자동!</p>
          </div>

          <div className="bg-[#F9FAFB] rounded-2xl p-4">
            <p className="font-bold text-sm mb-3">🏗️ 감가상각</p>
            <div className="space-y-2">
              <div className="bg-white rounded-lg p-2 text-center">
                <p className="text-xl">🔫</p>
                <p className="text-[10px] font-bold mt-1">장비 = 4년</p>
                <p className="text-[10px] text-[#9CA3AF]">에어소프트건류</p>
              </div>
              <div className="bg-white rounded-lg p-2 text-center">
                <p className="text-xl">🏢</p>
                <p className="text-[10px] font-bold mt-1">시설 = 10년</p>
                <p className="text-[10px] text-[#9CA3AF]">인테리어, PC</p>
              </div>
            </div>
          </div>
        </div>

        <div className="bg-[#FFFBEB] rounded-2xl p-4 flex items-start gap-3">
          <span className="text-xl">💡</span>
          <div>
            <p className="font-bold text-xs">에어소프트건 = "장비"로 등록하세요!</p>
            <p className="text-[11px] text-[#6B7280] mt-1">"재고"로 잡으면 팔 때 세금 폭탄, "장비"면 매년 조금씩 비용 처리돼서 절세!</p>
          </div>
        </div>
        <div className="flex gap-2">
          <GoBtn section="고정비" label="고정비" />
          <GoBtn section="감가상각" label="감가상각" />
        </div>
      </Card>

      {/* 데이터 저장 — 강조 */}
      <div className="bg-[#EF4444] rounded-3xl p-6 text-white">
        <p className="text-3xl mb-3">🚨</p>
        <h3 className="text-xl font-bold mb-2" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>제일 중요한 것!</h3>
        <p className="text-sm opacity-90 mb-4">이 앱은 자동 저장이 안 돼요.<br/>작업 끝나면 반드시 파일로 저장하세요!</p>

        <div className="bg-white/20 rounded-2xl p-4 mb-4">
          <div className="flex items-center gap-4 justify-center">
            <div className="text-center">
              <p className="text-2xl">📤</p>
              <p className="text-[10px] font-bold mt-1">데이터 메뉴</p>
            </div>
            <span className="text-xl">→</span>
            <div className="text-center">
              <p className="text-2xl">📥</p>
              <p className="text-[10px] font-bold mt-1">JSON 내보내기</p>
            </div>
            <span className="text-xl">→</span>
            <div className="text-center">
              <p className="text-2xl">✅</p>
              <p className="text-[10px] font-bold mt-1">저장 완료!</p>
            </div>
          </div>
        </div>

        <div className="space-y-2 text-sm">
          <p>📱 카카오톡 나에게 보내기 — 가장 간편!</p>
          <p>☁️ 구글 드라이브에 올려두기</p>
          <p>📧 세무사에게 이메일로 보내기</p>
        </div>
        <button onClick={() => onNavigate('데이터')}
          className="mt-4 px-5 py-2.5 bg-white text-[#EF4444] rounded-full text-sm font-bold hover:bg-white/90 transition">
          데이터 메뉴 바로가기 →
        </button>
      </div>
      ...
      {/* 월간 루틴 — 타임라인 비주얼 */}
      <Card>
        <p className="text-xs font-bold text-[#059669] mb-2">🗓️ 월간 루틴</p>
        <h3 className="text-lg font-bold mb-1" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>매달 10분이면 끝!</h3>
        <p className="text-xs text-[#9CA3AF] mb-4">매달 말에 한 번만 하면 돼요</p>

        <div className="space-y-3">
          {[
            { icon: '💳', title: '카드 전표 확인', time: '2분', desc: '단말기 합계 확인', sec: null },
            { icon: '📝', title: '매출 넣기', time: '2분', desc: '카드/현금 금액 입력', sec: '월별입력' },
            { icon: '💸', title: '변동비 넣기', time: '3분', desc: '접대비, 기름값 등', sec: '월별입력' },
            { icon: '🔫', title: '중고매입 기록', time: '2분', desc: '당근/중고나라 구매', sec: '중고매입' },
            { icon: '📊', title: '역산 확인', time: '1분', desc: '세금 목표 체크', sec: '대시보드' },
            { icon: '💾', title: 'JSON 저장!!!', time: '10초', desc: '꼭 하세요!!!', sec: '데이터' },
          ].map((step, i) => (
            <div key={i} className={`flex items-center gap-3 rounded-2xl p-3 transition-all ${i === 5 ? 'bg-[#FEF2F2] border-2 border-[#EF4444]' : 'bg-[#F9FAFB]'}`}
              onClick={() => step.sec && onNavigate(step.sec)}
              style={{ cursor: step.sec ? 'pointer' : 'default' }}>
              <span className="text-2xl">{step.icon}</span>
              <div className="flex-1 min-w-0">
                <p className={`font-bold text-sm ${i === 5 ? 'text-[#EF4444]' : ''}`}>{step.title}</p>
                <p className="text-[10px] text-[#9CA3AF]">{step.desc}</p>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                <span className="text-[10px] bg-white px-2 py-1 rounded-full text-[#6B7280]">{step.time}</span>
                {step.sec && <span className="text-[#9CA3AF] text-xs">→</span>}
              </div>
            </div>
          ))}
        </div>
      ...
      {/* FAQ — 심플 */}
      <Card>
        <p className="text-xs font-bold text-[#059669] mb-2">❓</p>
        <h3 className="text-lg font-bold mb-4" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>자주 하는 질문</h3>
        <div className="space-y-2">
          {faqs.map((faq, i) => (
            <div key={i} className="bg-[#F9FAFB] rounded-2xl overflow-hidden">
              <button onClick={() => setOpenFaq(openFaq === i ? null : i)} className="w-full flex justify-between items-center p-4 text-left">
                ...
                <span className={`text-[#9CA3AF] text-xs transition-transform duration-200 ${openFaq === i ? 'rotate-180' : ''}`}>▼</span>
              </button>
              {openFaq === i && (
                <div className="px-4 pb-4">
                  <p className="text-xs text-[#6B7280] bg-white rounded-xl p-3">{faq.a}</p>
                </div>
              )}
            </div>
          ))}
...
const App = () => {
  ...
  const renderSection = () => {
    switch (s.section) {
      ...
      case '도움말': return <HelpGuide onNavigate={(sec) => set(p => ({ ...p, section: sec }))} />;
